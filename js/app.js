...
const mostrarDatos = async (latitud, longitud) => {
  try {
    const api = "47594b049d13bba0a5d653c652298139";
    const datos = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?&units=metric&lat=${latitud}&lon=${longitud}&lang=es&appid=${api}`
    );
...
const mostrarDatosCiudad = async (ciudad, pais) => {
  try {
    const api = "47594b049d13bba0a5d653c652298139";
    const datos = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?&units=metric&q=${ciudad},${pais}&lang=es&appid=${api}`
    );
    const dataJson = await datos.json();
    sppiner2();
    if (dataJson.cod == "404") {
...
const limpiarHtml = () => {
  while (contenedorCiudad.firstChild)
    contenedorCiudad.removeChild(contenedorCiudad.firstChild);
};
